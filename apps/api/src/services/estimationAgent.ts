...
import { createEffortEstimate, RiskLevel } from "../stores/effortEstimates";
...
export function generateEstimation(req: EstimationRequest): { artifactId: string; content: EstimationContent; estimateIds: string[] } {
  ...
  const artifact = createArtifact({
    ...
    diagramUrl: null,
    parentArtifactId: req.artifactId ? req.artifactId : null,
    generatedBy: "ai",
  ...
  for (const it of items) {
    const est = createEffortEstimate({
