...
function mapDocument(d: any): DocumentEntity {
  return {
    ...
    doc_type: String(d.type), file_size: Number(d.fileSize ?? 0),
    parsedStatus: d.parsedStatus, storage_key: d.storageKey ?? d.storageUrl ?? null,
...
export class PostgresDocumentAggregateRepository implements IDocumentAggregateRepository {
  ...
  async createDocument(orgId: string, projectId: string, data: { filename: string; docType: string; fileSize: number; storageKey?: string }): Promise<DocumentEntity> {
    ...
    return withTenant(this.prisma as never, orgId, async (tx: unknown) => {
      ...
      const d = await p.document.create({ data: {
        orgId, projectId, filename: data.filename,
        type: data.docType, storageUrl: data.storageKey ?? "",
        parsedStatus: "pending", uploadedBy: undefined,
      } });
  ...
  async updateParsedStatus(orgId: string, id: string, status: "pending" | "parsed" | "failed"): Promise<DocumentEntity> {
    assertTenant(orgId);
    return withTenant(this.prisma as never, orgId, async (tx: unknown) => {
      const d = await (tx as any).document.update({ where: { id }, data: { parsedStatus: status } });
      ...
      for (const c of chunks) {
        ...
        if (vector) {
          await p.$executeRawUnsafe(
            "INSERT INTO document_chunks (id, document_id, org_id, chunk_text, embedding, page_ref) VALUES ($1::uuid,$2::uuid,$3::uuid,$4,$5::vector,$6)",
            id, documentId, orgId, c.content, vector, c.pageNumber ?? null,
          );
        } else {
          await p.$executeRawUnsafe(
            "INSERT INTO document_chunks (id, document_id, org_id, chunk_text, page_ref) VALUES ($1::uuid,$2::uuid,$3::uuid,$4,$5)",
            id, documentId, orgId, c.content, c.pageNumber ?? null,
          );
        }
  ...
  async searchSimilarChunks(orgId: string, projectId: string, queryEmbedding: number[], topK: number): Promise<Array<DocumentChunkEntity & { score: number }>> {
    ...
    return withTenant(this.prisma as never, orgId, async (tx: unknown) => {
      ...
      const rows = await p.$queryRawUnsafe(
        `SELECT c.id, c.org_id AS "orgId", c.document_id AS "documentId", c.chunk_text AS content,
                c.page_ref AS "page_number", c.created_at AS "createdAt",
                1 - (c.embedding <=> $1::vector) AS score
           FROM document_chunks c
           JOIN documents d ON d.id = c.document_id
          WHERE c.org_id = $2::uuid AND d.org_id = $2::uuid AND d.project_id = $3::uuid
            AND c.embedding IS NOT NULL
          ORDER BY c.embedding <=> $1::vector ASC
          LIMIT $4`, vector, orgId, projectId, limit,
      );
