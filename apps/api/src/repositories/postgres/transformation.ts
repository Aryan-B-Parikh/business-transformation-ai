...
export class PostgresTransformationAggregateRepository implements ITransformationAggregateRepository {
  ...
  async getJourneyState(orgId:string, projectId:string):Promise<JourneyStageEntity[]> {
    ...
    return withTenant(this.prisma as never, orgId, async (tx:unknown) => {
      ...
      if(!journey) return [];
      return journey.transitions.map(t=>({id:t.id,orgId,projectId,stage:t.toStage as JourneyStage,status:t.toStage===journey.currentStage?(journey.status==="blocked"?"blocked":"in_progress"):"completed",entered_at:t.timestamp,completed_at:t.toStage===journey.currentStage?null:t.timestamp,completed_by:t.toStage===journey.currentStage?null:t.actor,blocked_reason:t.toStage===journey.currentStage?((journey.status==="blocked"?journey.status:null)):null,stage_version:t.revision}));
    });
  }

  async transitionStage(orgId:string,projectId:string,stage:JourneyStage,status:JourneyStatus,userId?:string,blockedReason?:string):Promise<JourneyStageEntity> {
    assertTenant(orgId);
    return withTenant(this.prisma as never, orgId, async (tx:unknown) => {
      const p=tx as PrismaClient;
      ...
      if(!journey){
        ...
        return {id:journey.id,orgId,projectId,stage:"idea",status:status as JourneyStatus,entered_at:journey.enteredAt,completed_at:null,completed_by:null,blocked_reason:blockedReason??null,stage_version:1};
      }
