...
export function initializeRepositories(backend:StorageBackend=(process.env.STORAGE_BACKEND as StorageBackend)||(isTest()?"memory":"postgres"),prisma?:PrismaClientType):Repositories{
 ...
 if(backend==="postgres"){const client=prisma as PrismaClientType;activeRepositories={projects:new PostgresProjectRepository(client),artifacts:new PostgresArtifactRepository(client),transformation:new PostgresTransformationAggregateRepository(client as unknown as PrismaClient),documents:new PostgresDocumentAggregateRepository(client as unknown as PrismaClient),conversations:new PostgresConversationAggregateRepository(client as unknown as PrismaClient),collaboration:new PostgresCollaborationRepository(client),webhooks:new PostgresWebhookRepository(client),governance:new PostgresGovernanceRepository(client)};}
 else activeRepositories={projects:new MemoryProjectRepository(),artifacts:new MemoryArtifactRepository(),transformation:new MemoryTransformationRepository(),documents:new MemoryDocumentRepository(),conversations:new MemoryConversationAggregateRepository(),collaboration:new MemoryCollaborationRepository(),webhooks:new MemoryWebhookRepository(),governance:new MemoryGovernanceRepository()};
