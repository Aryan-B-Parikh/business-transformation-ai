...
router.get("/webhooks/deliveries",authenticate,authorize("org_admin","workspace_admin"),async(req:AuthedRequest,res)=>{
  const orgId=req.user!.orgId;
  const all = await getRepositories().webhooks.listPendingOutboxEvents(100);
  const evts = all.filter(x => x.orgId === orgId);
  // mock deliveries response from outbox events
  return res.json({data: evts.map(e => ({ id: e.id, event: e.event_type, payload: e.payload, status: e.status }))});
});
